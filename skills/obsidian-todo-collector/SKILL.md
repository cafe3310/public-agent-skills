...
从早到晚，依次处理每个文档（下面称为「当前文档」），执行下列循环：

1. 确定当前批次的「当前文档」：通过文件名筛选出指定时间范围内的文档，按照日期顺序处理。
2. 预检当前文档：使用 Skill 提供的 Python 脚本进行预检，确保文档符合处理条件。
    ```bash
...
5. 对「目标文档」用多次批量 `replace`，加入新的待办事项列表，并编辑状态有更新的事项。
6. 更新「目标文档」的「当前状态」部分，给已经处理完成的文档打已完成 [x] 。这样可以清晰地知道已经处理了哪些文档，哪些文档还未处理。
7. 跳回 1，继续处理下一个文档，直到处理完指定时间范围内的所有文档。
...
- 对于「目标文档」，再你不确定时，尽量先读再写。我会在合适时机 compress 或断点续传，你无须节省 token。
